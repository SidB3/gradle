...
class TestNGCoverage {
    ...
    static final Set<String> SUPPORTS_PRESERVE_ORDER
    static final Set<String> SUPPORTS_GROUP_BY_INSTANCES
    static final Set<String> SUPPORTS_ICLASS_LISTENER

    static {
        SUPPORTED_BY_JDK = testNgVersionsSupportedByJdk(VERSIONS, JavaVersion.current())
        SUPPORTS_PRESERVE_ORDER = SUPPORTED_BY_JDK.findAll { VersionNumber.parse(it) >= VersionNumber.parse('5.14.5') }
        SUPPORTS_GROUP_BY_INSTANCES = SUPPORTED_BY_JDK.findAll { VersionNumber.parse(it) >= VersionNumber.parse('6.1') }
        SUPPORTS_ICLASS_LISTENER = SUPPORTED_BY_JDK.findAll { VersionNumber.parse(it) >= VersionNumber.parse(FIXED_ICLASS_LISTENER) }
    }
